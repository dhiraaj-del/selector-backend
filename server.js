...
function dodoActivate(licenseKey, name) {
  return new Promise((resolve) => {
    const body = JSON.stringify({ license_key: licenseKey, name: name });
    const options = {
    ...
    req.setTimeout(10000, () => { req.destroy(); resolve({ status: 408, data: {} }); });
    req.write(body);
    req.end();
  });
}
...
app.get('/', (req, res) => {
  res.json({ status: 'ok', app: 'Selector License Backend', version: '3.0.0' });
});
...
app.post('/webhook/dodo', async (req, res) => {
  try {
    ...
    if (event.type === 'payment.succeeded') {
      ...
      if (!email) { console.log('[Dodo] No email found'); return res.json({ ok: true }); }
      const existing = await pool.query('SELECT * FROM licenses WHERE order_id=$1', [payment_id]);
      if (existing.rows.length > 0) { console.log('[Dodo] License already issued'); return res.json({ ok: true }); }
      const licenseKey = generateLicenseKey();
      await pool.query('INSERT INTO licenses (id, key, email, order_id, payment_id) VALUES ($1,$2,$3,$4,$5)', [uuid(), licenseKey, email, payment_id, payment_id]);
      try { await sendLicenseEmail(email, licenseKey, payment_id); console.log('[Dodo] License', licenseKey, 'sent to', email); }
      catch (e) { console.error('[Dodo Email Error]', e.message); }
    }
    res.json({ ok: true });
  } catch (e) {
...
app.post('/validate-dodo-key', async (req, res) => {
  try {
    ...
    const msg = (result.data && result.data.message) || 'Invalid license key';
...
app.post('/validate-key', async (req, res) => {
  try {
    ...
    if (result.rows.length === 0) return res.json({ valid: false, error: 'License key not found' });
    const license = result.rows[0];
    if (!license.activated) {
      await pool.query('UPDATE licenses SET activated=1, instance_id=$1, activated_at=NOW() WHERE key=$2', [instanceId || 'unknown', key]);
    }
...
app.get('/stats', async (req, res) => {
  ...
  res.json({ totalLicenses: count, activatedLicenses: parseInt(activated.rows[0].count), revenue: 'INR ' + (count * 1999) });
});
